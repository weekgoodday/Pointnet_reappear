...
class ScannetDatasetWholeScene():
    ...
    def __init__(self, root, block_points=4096, split='test', test_area=5, stride=0.5, block_size=1.0, padding=0.001):
        ...
        if self.split == 'train':
            ...
            # a=[d for d in os.listdir(root)]
            # print("a is :",a)
            print("file_list is: ",self.file_list)
        self.scene_points_list = []
        self.semantic_labels_list = []
        self.room_coord_min, self.room_coord_max = [], []
        for file in self.file_list:
            data = np.load(root + file) #这个就是保存下来的7维数据
            points = data[:, :3] #点云只取前三维
            self.scene_points_list.append(data[:, :6]) #用的都是6维
            self.semantic_labels_list.append(data[:, 6]) #最后一维是label
            coord_min, coord_max = np.amin(points, axis=0)[:3], np.amax(points, axis=0)[:3]
            self.room_coord_min.append(coord_min), self.room_coord_max.append(coord_max) #amin amax是array的最小最大，取每列的最小最大框出bound
        assert len(self.scene_points_list) == len(self.semantic_labels_list) #断言点云数据和label一样多
